...
// 项目
def project_name = "local-standard-address-frontend"
def registry_name = "standard-address-frontend"
def image_name = "${registry}/${registry_name}/${project_name}:${BUILD_NUMBER}"
def gitlab_url = "http://gitlab-ce-svc.devops.svc.cluster.local/bzdz/standard-address-frontend.git"
...
spec:
  ...
  - name: build
    image: 192.168.31.154:8443/devops/node:16.2.0-alpine
    imagePullPolicy: IfNotPresent
    tty: true
    command:
      - "cat"
              ...
              stage("代码编译") {
                  steps {
                      container(name: "build") {
                          ...
                          npm config set cache /data/node-cache
                          npm install
                          npm run build
                          ls
                          """
                  ...
                  steps {
                      container(name: "sonarqube") {
                        withCredentials([string(credentialsId: "${sonarqube_token}", variable: "SONAR_TOKEN")]) {
                          ...
                          -Dsonar.sources=src \
                          -Dsonar.exclusions=**/*_test.js,**/vendor/** \
                          -Dsonar.test.inclusions=client/**/**_spec.js,client/**/**_spec.jsx,client/**/**_test.js,client/**/**_test.jsx,client/**/**_spec.ts,client/**/**_spec.tsx,client/**/**_test.ts,client/**/**_test.tsx \
                          -Dsonar.typecript.lcov.reportPaths=coverage/lcov.info \
